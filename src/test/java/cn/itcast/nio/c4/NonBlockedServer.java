...
public class NonBlockedServer {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ssc.configureBlocking(false); // 将 ServerSocketChannel 设置为非阻塞模式
        ...
        while (true) {
            ...
            SocketChannel sc = ssc.accept(); // 非阻塞模式下，如果没有新连接建立，返回 null，线程不会死等
            if (sc != null) {
                sc.configureBlocking(false); // 将 SocketChannel 设置为非阻塞模式
            ...
            for (SocketChannel channel : channels) {
                // 5. 接收客户端发送的数据
                int read = channel.read(buffer);// 非阻塞模式下，如果在当前 channel 没有接收到数据，返回 0，线程不会死等
                if (read > 0) {
                    buffer.flip();
                    debugRead(buffer);
                    buffer.clear();
